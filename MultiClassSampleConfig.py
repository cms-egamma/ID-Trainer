...
####Start here
#####################################################################
OutputDirName = 'MultiClassOutput' #All plots, models, config file will be stored here
...
branches=["ele_*","matchedToGenEle","scl_eta"]
#Possible examples
# ["ele_*","matchingflag",]
# ["ele_pt", "ele_deltaEtaSC", "ele_r9","ele_eta"]
...
Classes = ['Class1','Class2','Class3']#Which classes are you going to use (Do not modify this if you don't have a reason to)
ClassColors = ['#377eb8', '#ff7f00', '#4daf4a'] # To use in plots

#dictionary of processes
#DYJetsToLL_M-50_v7_ElePromptGenMatched.root  DYToEE_M-50.root  GJet_Pt-20toInf_DoubleEMEnriched_MGG-40to80.root  QCD_Pt_15to7000.root
processes=[
    {#Class 1 block
        'Class':'Class1',
        'path':'GJet_Pt-20toInf_DoubleEMEnriched_MGG-40to80.root', #path of root file
    ...
    {#Class 2 block
        'Class':'Class2',
        'path':'DYToEE_M-50.root', #path of root file
        'xsecwt': 1, #xsec wt if any, if none then it can be 1
        'CommonSelection':CommonCut, #Common selection for all classes
    ...
    {#Class 3 block
        'Class':'Class3',
        'path':'QCD_Pt_15to7000.root', #path of root file
        'xsecwt': 1, #xsec wt if any, if none then it can be 1
        'CommonSelection':CommonCut, #Common selection for all classes
        'selection':"(matchedToGenEle >= 0)"#selection for signal
    },#Class 3 block ends
...
MVAs = [
    ...
    {"MVAtype":"XGB_1", 
     ...
     "feature_bins":[100]*17, #same length as features"feature_bins":[100, 100, 100], #same length as features
    ...
    {"MVAtype":"DNN_1",
     ...
     "features":["ele_fbrem", "ele_deltaetain", "ele_deltaphiin", "ele_oldsigmaietaieta",
                 ...
                 "ele_dr03EcalRecHitSumEt","ele_dr03HcalTowerSumEt","ele_gsfchi2","scl_eta","ele_pt"],
     #Input features #Should be branchs in your dataframe
     "feature_bins":[100]*17, #same length as features
     #Binning used only for plotting features (should be in the same order as features), does not affect training
     'Scaler':"MinMaxScaler",
    ...
    {"MVAtype":"DNN_2",
     ...
     "Label":"DNN mass", # label can be anything (this is how you will identify them on plot legends)
     "features":["ele_fbrem", "ele_deltaetain", "ele_deltaphiin", "ele_oldsigmaietaieta",
                 "ele_oldhe", "ele_ep", "ele_olde15", "ele_eelepout",
                 "ele_kfchi2", "ele_kfhits", "ele_expected_inner_hits","ele_dr03TkSumPt",
                 "ele_dr03EcalRecHitSumEt","ele_dr03HcalTowerSumEt","ele_gsfchi2"],
     #Input features #Should be branchs in your dataframe
     "feature_bins":[100]*15, #same length as features
     #Binning used only for plotting features (should be in the same order as features), does not affect training
     'Scaler':"MinMaxScaler",
     "DNNDict":{'epochs':10, 'batchsize':1000,
                'model': Sequential([Dense(48, kernel_initializer='glorot_normal', activation='relu'),
                                     Dense(24, activation="relu"),
                                     Dense(24, activation="relu"),
                                     Dropout(0.1),
                                     Dense(len(Classes))]),
                'compile':{'loss':'binary_crossentropy','optimizer':Adam(lr=0.01), 'metrics':['accuracy']},
                'earlyStopping': EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=10)
               }
    },
    ...
    {"MVAtype":"XGB_2", 
     ...
     'XGBGridSearch':{'learning_rate':[0.1],      
